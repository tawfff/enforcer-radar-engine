...
const TOOL = /\b(mcp server|model context protocol|coding agent|agent framework|agent skills|llm[ -]?agent|langgraph|langchain|prompt injection|burp suite|claude code|ai agents?|gpg|pgp key|keygen|skip-invite|behavioral assurance)\b/i;
const VENDOR = /^(persona|plaid|privy|onfido|sumsub|sumsubstance|innovatrics|doubangotelecom|faceonlive|kby-ai|veriff|auth0|okta|workos|clerkinc|complycube)\//i;
